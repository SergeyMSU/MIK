...
!******7**10**********************************************************70
 ...
 !   end interface
	...
	contains
	  ...
	  integer, intent(in), optional :: n_disc
	  real(8), intent(in out), optional :: konvect_(:, :)  ! (3, :) ƒл€ конвективного сноса различных переменных, второй аргумент, это их количество
...
!c----
  ...
  !      qqq(i+1)=aco(i,1)*qv(1)+aco(i,2)*qv(2)+aco(i,3)*qv(3)
	...
	!@cuf attributes(host) & 
    subroutine Calc_sourse_MF(plasma, fluid, sourse, zone)  ! —читаютс€ мультифлюидные источники
    ! Variables
    use GEO_PARAM, only: par_a_2, par_n_p_LISM, par_Kn, par_pi_8, par_n_H_LISM_
    implicit none
    real(8), intent(in) :: plasma(9)
    real(8), intent(in) :: fluid(5,4)
    real(8), intent(out) :: sourse(5,5)  ! (масса, три импульса и энерги€)
