...
module MY_CUDA
 ...
 !     dimension qqq(8),qqq1(8),qqq2(8)
	...
	subroutine CUDA_START_MGD_move_MK()
	! На подвижной сетке! с источниками из Монте-Карло
	use STORAGE
    use GEO_PARAM
	use MY_CUDA
	implicit none
 ...
 !   dev_gl_Gran_square = dev_gl_Gran_square2(:, now2)
	...
	if (mod(step, 25000) == 1 .or. step == 5000) then
		...
		do ijk = 1, 6000  ! Несколько раз просчитываем внутреннюю область
	...
	if (mod(step, 55000) == 0 .or. step == 1000 .or. step == 5000 .or. step == 10000 .or. step == 15000 .or. step == 30000) then
	...
	end subroutine CUDA_START_MGD_move_MK
