    ...
    real(4), parameter :: par_R_character = 0.2         ! Характерный размер в задаче (расстояние до TS на начальном этапе построения сетки)
    real(4), parameter :: par_R0 = 0.002564         ! Характерный размер 1 а.е. (внутренней сферы)
    ! параметр нужен для геометрической точности сетки eps = par_R_character/10000
    real(4), parameter :: par_R_END = 3.7         ! 
    real(4), parameter :: par_R_LEFT = -3.8         ! 
    real(8), parameter :: par_pi_8 = acos(-1.0_8)         ! Характерный размер в задаче (примерное расстояние до TS)
    real(4), parameter :: par_pi_4 = acos(-1.0_4)         ! Характерный размер в задаче (примерное расстояние до TS)
    real(8), parameter :: cpi4 = 12.56637061435917295384
    real(8), parameter :: ggg = (5.0/3.0)
    
    real(8), parameter :: par_chi_real = 1.6
    real(8), parameter :: par_Velosity_inf = -2.54338
    
    real(4), parameter :: par_kk1 = 1.3     ! Степень сгущения сетки к нулю в области до TS: 1 - линейное, 2 - квадратичное и т.д.
    
    integer :: par_l_phi = 80 !4   ! Количество вращений двумерной сетки для получения трёхмерной (разрешение по углу phi)
                                ! Должно делиться на 4 для удобного вывода результатов в плоскостях
    integer :: par_m_A = 30 !5     ! Количество лучей A в плоскости
    integer :: par_m_BC = 20 !3     ! Количество лучей B/C в плоскости
    integer :: par_m_O = 30 !7 !3     ! Количество лучей O в плоскости
    integer :: par_m_K = 7 !2     ! Количество лучей K в плоскости
    real(4) :: par_triple_point = 13.0 * par_pi_4/40.0     ! До какого угла начиная от pi/2 (с положительного x) тройная точка
    
    ! Количество точек по лучам A
    integer :: par_n_TS =  25! 3                 ! Количество точек до TS (TS включается)
    integer :: par_n_HP =  40! 4                 ! Количество точек HP (HP включается)  всё от 0 считается
    integer :: par_n_BS =  55! 5                 ! Количество точек BS (BS включается)
    integer :: par_n_END = 75! 6                ! Количество точек до конца сетки (конец включается)
    ...
    real(4), allocatable :: gl_x(:)   ! набор x-координат узлов сетки 
    real(4), allocatable :: gl_y(:)   ! набор y-координат узлов сетки 
    real(4), allocatable :: gl_z(:)   ! набор z-координат узлов сетки 
    ...
    real(4), allocatable :: gl_Cell_dist(:)             ! Минимальное расстояние до грани в каждой ячейки  4444444444444444
    real(4), allocatable :: gl_Cell_center(:, :)             ! (3, :) Центр каждой ячейки  4444444444444444
    ...
    real(4), allocatable :: gl_Gran_normal(:,:)       ! (3, :) Нормаль грани   4444444444444444
