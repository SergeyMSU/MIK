...
attributes(global) subroutine Cuda_Move_all_2(now)   ! Поверхностное натяжение на лучах B
	...
	! Ударная волна
		...
		if (gl_Point_num(yzel) > 0) then
			vel = 0.5 * gl_Point_num(yzel) * par_nat_TS * (Ck/2.0 + Ek/2.0 - Ak)/Time * ddt  ! 10.0   ->  1.0
		else
			vel = 0.5 * par_nat_TS * (Ck/2.0 + Ek/2.0 - Ak)/Time * ddt
		end if
	...
	!	Ck(1), Ck(2), Ck(3))
		...
		if (gl_Point_num(yzel) > 0) then
			...
			if (gl_Point_num(yzel) > 0) then
				...
				vel = kkk * par_nat_HP * 0.0006 * 0.5 * ((Ek/2.0 + Ck/2.0 - Ak) + (Bk/2.0 + Dk/2.0 - Ak))/2.0 * gl_Point_num(yzel)/Time  !0.000003 
			else
				...
				vel = kkk * par_nat_HP * 0.0006 * 0.5 * ((Ek/2.0 + Ck/2.0 - Ak) + (Bk/2.0 + Dk/2.0 - Ak))/2.0 /Time  !0.00006
			end if
		end if
	...
	if (s2 >= 1 .and. par_TVD == .True. .and. gl_Gran_type(gr) /= 2) then
			...
			if (ss1 /= 0 .and. ss2 /= 0) then
				...
				if(tvd1 == .True. .and. tvd2 == .False.) then
					...
					call dekard_skorost(gl_Gran_center2(3, gr, now), gl_Gran_center2(1, gr, now), gl_Gran_center2(2, gr, now), &
						aa, bb, cc, qqq2_TVD(4), qqq2_TVD(2), qqq2_TVD(3))
